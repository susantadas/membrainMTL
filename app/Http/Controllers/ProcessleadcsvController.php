...
class ProcessleadcsvController extends Controller
{
    ...
    public function index() {
        if(Auth::user()->role_id==1 || Auth::user()->role_id==2) {
            $clients = DB::table('clients')->where('active','=','1')->orderBy('name', 'asc')->get();
        ...
        if($request->hasFile('myfile')){
            ...
            if($public_id[0]->public_id != $newfile_name){
                ...
                if(filesize($file_path)) {
                    $empty=1;
                    $extfilenameforpcsv = pathinfo($_FILES['myfile']['name'], PATHINFO_EXTENSION);
                    if($extfilenameforpcsv=='txt'){
                    ...
                    } else if($extfilenameforpcsv=='csv'){                        
                        $CountData =  Excel::load($file_path, function($reader){
                            $results = $reader->noHeading();
                        })->toArray();
                        foreach ($CountData as $key => $value) {
                            $regExp = '/[\t]/';
							$newvalue = preg_split($regExp, $value[0]);
                            if(!empty($value)){
                                if(count($value) > 1){
                                    $delimiter = 1;
    ...
    public function csvProcessingFromPortal(Request $request){
        try{
            ...
            if($request->input('camp_id')!=''){
                $campaign_id = $request->input('camp_id');
            } else {
                $commendLine = func_get_args();
                $campaign_id = $commendLine[1];
            }
            ...
            if (strpos($filename_campaign_id, $public_id) !== false) {
                ...
                $current_sup_name = "empty(because no supplier_id)";
                ...
                $_curr_camp_param_map_required = $campaign[0]->parameter_required;
                $curr_camp_param_map_required = json_decode($_curr_camp_param_map_required);

                $curr_camp_public_id = $campaign[0]->public_id;
                if($campaign[0]->method=='CSV'){
                    if($filename_campaign_id == $campaign[0]->public_id){
                        $Response .= "<br />File found is: ".$latest_filename;
                        $delimiter = $this->getFileDelimiter($path, $latest_filename, 2);
                        $Response .= "<br />Delimiter is: ".$delimiter;
                        if($delimiter == '\t' && $delimiter!='0'){
                            $Response .= "<br />File is Tab Delimited.\n";
                            ...
                            $_fileextnewmyfile = explode('.',$latest_filename);
                            $fileextnewmyfile = end($_fileextnewmyfile);
                            if($fileextnewmyfile=='txt'){
                            ...
                            $Response .= "<br />count of mappings_issue: ".count($mappings_issue);
                            $Response .= "<br />count of mappings: ".count($mappings);
                            $successful_lead=0;
                            $invalid_lead=0;
                            $duplicate_lead=0;
                            $arr_res=array();
                            foreach ($mappings as $key => $each_mapping) {
                                ...
                                $datanew = array();
                                unset($each_mapping['supplier_id']);
                                unset($each_mapping['client_id']);
                                unset($each_mapping['campaign_id']);
                                if($result['lead_disposition']['success'] == 'true'){
                                    ...
                                    if(is_array($result['lead_disposition']['error'])  && (in_array('Duplicate Consumer',$result['lead_disposition']['error']) || in_array('duplicate consumer',$result['lead_disposition']['error']))){
                                        unset($result['lead_disposition']);
                                        unset($result['audi_flag']);
                                        unset($each_mapping['audi_flag']);
                                ...
                                $f_detection_data = array("supplier_id"=>$current_sup_id,"source"=>$latest_filename,"client_id"=>$current_client_id,"campaign_id"=>$current_camp_id,"email_address"=>(isset($result['email']) ? $result['email'] : ''),"phone"=>(isset($result['phone']) ? $result['phone'] : ''),"first_name"=>(isset($result['firstName']) ? $result['firstName'] : ''),"last_name"=>(isset($result['lastName']) ? $result['lastName'] : ''),"postcode"=>(isset($result['postcode']) ? $result['postcode'] : ''),"received"=>date('Y-m-d H:i:s'));
                            ...
                            if(!empty($return_csv)){
                                Excel::create($return_csv_filename, function($excel) use ($ret_csv_original_rec) {
                                    $excel->sheet('Return-CSV', function($sheet) use ($ret_csv_original_rec) {
                                        $sheet->fromArray($ret_csv_original_rec, null, 'A1', true, true);
                                    });
                            ...
                            if(!empty($client_csv)){
                                Excel::create($client_csv_filename, function($excel) use ($cli_csv_original_rec) {
                                    $excel->sheet('Client-CSV', function($sheet) use ($cli_csv_original_rec) {
                                        $sheet->fromArray($cli_csv_original_rec, null, 'A1', true, true);
                                    });
                                })->store('csv',storage_path('process_dir/portal_csv_upload/temp'));
                            }
                            ...
                            if($percent_result > $current_sup_error_count){
                                $Response .= "<br />The ratio of invalid leads exceeds the “error_allowance” threshold (defined in the “supplier” table)";
                                ...
                                if($alert->save()){
                                    $Response .= "<br />Alert record generated.";
                                }
                                ...
                                if($quarantine->save()){
                                    $Response .= "<br />Quarantine record generated, File Quarantined.";
                                }
                                ...
                                if($current_sup_return_csv == 1){
                                    ...
                                    File::delete($dest_path.'/'.$new_ret_csv_filename);
                                ...
                                File::delete($ret_csv_file_path_qua);
                                ...
                                if( count(\Mail::failures()) > 0 ) {
                                    $Response .= "<br />Some problem occurred. Email has not been sent to Supplier.";
                                } else {
                                    //echo "\nEmail has been sent to Supplier.";
                                    $Response .= "<br />Email has been sent to Supplier.";
                                }
                                /* END: Sending Email to supplier */                                
                                return json_encode(array('status'=>1, 'responseData'=>$Response));                 
                            }else{
                                $Response .= "<br />The error rate does not exceed the threshold";
                                if(isset($current_camp_server_param->server) && $current_camp_server_param->server!='' && isset($current_camp_server_param->user) && $current_camp_server_param->user!='' && isset($current_camp_server_param->password) && $current_camp_server_param->password!=''){
                                    if(isset($current_camp_server_param->directory) && $current_camp_server_param->directory != ''){
                                        ...
                                        if(File::exists($cli_csv_file_path)){
                                            if($current_camp_server_param->type=='ftp') {
                                                try{
                                                    $conn_id = ftp_connect($host, 21);
                                                    if(!$conn_id){
                                                        $Response .="<br />FTP connection failed.";
                                                    }
                                                    $login = ftp_login($conn_id,$usr,$pwd);
                                                    if ((!$conn_id) || (!$login)) {
                                                        $Response .="<br />FTP connection has failed!";
                                                    } else {
                                                        $fff = ftp_pasv($conn_id, true);
                                                        $upload = ftp_put($conn_id, $ftp_path.$client_csv_filename, $local_file, FTP_ASCII); 
                                                        if($upload == '1'){
                                                            $Response .="<br />Client-CSV hasbeen uploaded successfully";
                                                        } else {
                                                            $Response .="<br />Client-CSV upload Failed";
                                                        }
                                                    }
                                                    ftp_close($conn_id);
                                                } catch (\Exception $e) {
                                            ...
                                            } else if($current_camp_server_param->type=='sftp') {
                                                try{
                                                    $sftp = new SFTP($host);
                                                    if (!$sftp->login($usr,$pwd)) {
                                                        throw new Exception('Login failed');
                                                    } else {
                                                        if($sftp->put($ftp_path.$client_csv_filename,file_get_contents($local_file))){
                                                    ...
                                                    if($e->getMessage()!=''){
                                                        $Response .="\n Client SFTP connection has failed!";
                                                    }
                                                } 
                                            } else {
                                            }
                                            File::delete($cli_csv_file_path);
                                        }
                                    }else{
                                        $Response .="<br />path missmatch, File cannot be uploaded. Directory not set.";
                                    }
                                }
                            ...
                            $Response .= "<br />File is not Tab delimited.";
                            /* generating alert */
                            $alert = new Alert;
                            $alert->supplier_id = $current_sup_id;
                            ...
                            $alertSourcefile = storage_path('frauddetections/'.$current_sup_id);
                            File::makeDirectory($alertSourcefile, $mode = 0777, true, true);
                            ...
                            File::copy($dest_path.'/'.$latest_filename, $alertSourcefile.'/'.$latest_filename);
                            /* Sending Mail to supplier */
                            //$emailid = $current_sup_email; 
                            File::delete($source_path.'/'.$latest_filename);
                            $emailid = 'mr.anupamroy@gmail.com';
                    ...
                    return json_encode(array('status'=>0, 'responseData'=>'Campaingn method is not CSV')); 
                }
            }
        } catch (\Exception $e) {
            if($e->getMessage()!=''){
                return json_encode(array('status'=>0, 'responseData'=>$e->getMessage())); 
            }
        }
    ...
    public function csvprocessing(){
        try{
            set_time_limit(0);
            $suppliers = DB::table('suppliers')->where('active','=','1')->select('id','public_id','name','contact_email','error_allowance','return_csv')->get();
            $dir_counter = 0;
            foreach($suppliers as $each_sup){
                ...
                if(is_dir($path) && ! $this->is_dir_empty($path)) {
                    ...
                    foreach($campaigns as $each_cam){
                        ...
                        $_curr_camp_param_map_required = $each_cam->parameter_required;
                        $curr_camp_param_map_required = json_decode($_curr_camp_param_map_required);
                        $curr_camp_public_id = $each_cam->public_id;
                        if($each_cam->method=='CSV'){
                            if($filename_campaign_id == $each_cam->public_id){
                                $tab_delimited = $csv_process->check_if_file_is_tab_delimited($path, $latest_filename, 2);
                                if($tab_delimited == '1'){
                                    ...
                                    $_mynewFileforreturn = 'supplier_files/'.$current_sup_pub_id.'/download/';
                                    $mynewFileforreturn = storage_path($_mynewFileforreturn);
                                    File::makeDirectory($mynewFileforreturn, $mode = 0777, true, true);
                                    ...
                                    if($percent_result > $current_sup_error_count){
                                        ...
                                        $return_csv_filename = $curr_camp_public_id.'_Return-CSV.csv';
                                        $new_ret_csv_filenameOut = $_fileextnewmyfile[0].'_out.csv';
                                        File::delete($dest_path.'/'.$new_ret_csv_filenameOut);
                                        $camp_id_ret_csv_path_qua = 'process_dir/temp';
                                        $source_path = storage_path($camp_id_ret_csv_path_qua);
                                        $move = File::copy($source_path.'/'.$return_csv_filename, $dest_path.'/'.$new_ret_csv_filenameOut);
                                        ...
                                        $alert->body = "Supplier name: ".$current_sup_name."~Campaign name: ".$current_camp_name."~Client name: ".$current_client_name;
                                        //$alert->filename = $latest_filename;
                                        $alert->filename = $new_ret_csv_filenameOut;
                                        $alert->login_usernme = '';
                                        $alert->acknowledged = '1';
                                        $alert->created = date('Y-m-d H:i:s');
                                        if($alert->save()){
                                        ...
                                        $quarantine->reason = 'Excessive Errors';
                                        //$quarantine->filename = $latest_filename;
                                        $quarantine->filename = $new_ret_csv_filenameOut;
                                        $quarantine->created = date('Y-m-d H:i:s');
                                        if($quarantine->save()){
                                        ...
                                        File::makeDirectory($ret_csv_quarantine_path, $mode = 0777, true, true);
                                        File::cleanDirectory($ret_csv_quarantine_path);
                                        File::move($ret_csv_file_path_qua, $ret_csv_quarantine_path.'/'.$new_ret_csv_filenameOut);
                                        /* END: Return CSV is moved to the quarantine directory for manual quarantine processing */
                                        ...
                                        if($current_sup_return_csv == 1){
                                            ...
                                            File::makeDirectory($dest_path, $mode = 0777, true, true);
                                            File::cleanDirectory($dest_path);
                                            File::copy($ret_csv_quarantine_path.'/'.$new_ret_csv_filenameOut, $dest_path.'/'.$new_ret_csv_filenameOut);
                                        }
                                        File::delete($ret_csv_file_path_qua);
                                        File::delete($file_path);
                                        /* START: Sending Email to supplier */
                                                    ...
                                                    if($current_camp_server_param->type=='ftp'){
                                                        ...
                                                        try{
                                                            ...
                                                            if (!$sftp->login($usr,$pwd)) {
                                                                ...
                                                                if($sftp->put($ftp_path.$client_csv_filename,file_get_contents($local_file))){
                                                                    $Response .="\nClient-CSV upload successful\n";
                                                                } else {
                                                                    $Response .="\ncould not upload  Client-CSV\n";
                                                                } 
                                                            }
                                                        } catch (\Exception $e) {
                                                            if($e->getMessage()!=''){
                                        ...
                                        if($current_sup_return_csv == 1){
                                            ...
                                            File::makeDirectory($dest_path, $mode = 0777, true, true);
                                            File::cleanDirectory($dest_path);
                                            File::copy($ret_csv_file_path_qua, $dest_path.'/'.$new_ret_csv_filenameOut);
                                        } 
                                        File::delete($ret_csv_file_path_qua);
                                        File::delete($file_path);
                                        /* START: Sending Email to supplier */
