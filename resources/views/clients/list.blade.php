                    ...
                    <tbody>
                        @foreach($clients as $key => $client)
                            <tr class="active" id="client-id-{{$client->id}}">
                                ...
                                <td>{{$client->PortalUser + $client->sPortalUser}}</td>
                        ...
                        if (confirm('Are you sure you want to delete this?')) {
                            jQuery.ajax({
                                ...
                                success: function(result){
                                    if(result==1){
                                        jQuery('#success-errors').html('<div class="alert alert-success"><span class="glyphicon glyphicon-ok"></span><em>Client User has been deleted</em></div>');
