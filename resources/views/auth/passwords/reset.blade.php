    ...
    <div class="app-cam">
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function(){
                jQuery.ajax({ 
                    url: "{{ action('HomeController@invalidReset') }}",
                    type:"post",
                    data:{'_token':'{{ csrf_token() }}','username':"{{ app('request')->input('email') }}"}, 
                    success: function(response){
                        if(response==0){
                            jQuery('.app-cam').html('<div class="xs"><div  class="alert alert-danger" >This URL has expired</div></div>')
                        }
                    }
                });
            });
